...
>
> **Post-cierre, 2026-08-14 — D19 tenía una segunda instancia, en el párrafo escrito para evitarla.**
> Auditando `templates/generated-app-CLAUDE.md` apareció que el párrafo que le prohíbe a la API fijar
> su dirección **cerraba afirmando que "el puerto sale del perfil de arranque, que ya está en el
> esqueleto"**. `templates/scaffold/` no contiene ningún `launchSettings.json`: la oración era falsa, y
> era exactamente la forma que D19 acababa de diagnosticar —un dato contingente del entorno, en
> indicativo, dentro de una plantilla que el agente lee como especificación—, escrita a dos renglones
> de la regla que la prohíbe. Se borró. Lo general, que es lo incómodo: **enunciar la regla no la
...
>
> **Y verificarlo costó tres corridas, con un defecto distinto cada una — el tercero es el que valía.**
> La primera cumplió CA-14 y CA-15 sin discusión (el frontend generado trae formulario, alta y baja de
> dependencias) pero la API filtró un value object de dominio al JSON: el `id` salía como
> `{"valor": "<guid>"}` en vez de un string. La segunda lo corrigió sola y expuso el de abajo:
> **crear una tarea devolvía un id, y listarla enseguida devolvía la misma tarea con otro id.** La
> causa está en `TaskItem.Create()`, que siempre inventa identidad: el dominio nunca expuso una vía
> de reconstitución, el agente de API no podía escribirla —`src/Domain/` es de solo lectura para él,
> y el hook lo bloquea— y al tercer intento resolvió los diagnostics de la única forma que compilaba,
> llamando a `Create()`. **Los tres gates en verde y la aplicación inservible más allá del primer
> request.** Es **ADR-020**, y es la segunda instancia de la misma familia que la evidencia de D16:
> la frontera donde este pipeline falla no son las reglas de negocio, es la reconstitución del
> agregado. El agravante que la decisión deja escrito: `api.md` le manda al agente *reportar* lo que
> falta en la capa de abajo, y **el grafo por diseño no lee lo que el agente responde** (ADR-004), así
> que ese reporte no tiene destinatario — deuda D21.
...
| D20 | **La prohibición de fijar una dirección se le impone a la API y no al frontend, y el pipeline funciona por coincidencia.** `templates/generated-app-CLAUDE.md` obliga a la API a recibir su dirección del orquestador (`app.Run()` pelado), pero ni esa plantilla ni `templates/agents/frontend.md` le dicen al agente de frontend cómo alcanzar una API cuya dirección elige el orquestador. El agente hace lo único que puede: la inventa — `const API_BASE_URL = "http://localhost:5000/api/tareas"` en el `App.tsx` generado | 2026-08-14, auditando la plantilla tras D19 | **Anda porque sin `launchSettings.json` el default de Kestrel es 5000, no porque nadie lo haya decidido.** El día que la API arranque en otro puerto, el frontend queda apuntando a la nada — y no hay gate que lo note, porque no existe `frontend-runtime` (D17). El arreglo es un proxy de Vite hacia un origen único, o la dirección por variable de entorno del build; las dos son cambios de `templates/scaffold/`, o sea aparato de medición y no producto (ADR-016) |
| D21 | **Un agente que descubre que le falta algo en la capa de abajo no tiene a quién decírselo.** `templates/agents/api.md` le manda reportarlo en vez de implementarlo —correcto: implementarlo duplicaría la invariante— pero el grafo no lee lo que el agente responde, por diseño (ADR-004). La instrucción se cumple y no produce ningún efecto | 2026-08-17, ADR-020 | Cuando aparezca la tercera instancia de un hueco de capa inferior que el agente detectó y nadie escuchó. El arreglo **no** es que el grafo lea prosa: sería un dato tipado que el agente emite y una arista que devuelva el turno a la capa dueña. Es diseño real, del tamaño de un ADR propio, y por eso no se abrió para destrabar una corrida |
