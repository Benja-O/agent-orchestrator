...
7. **Una entidad con identidad generada necesita una vía de vuelta desde la persistencia.** La capa de API guarda tus entidades y las **reconstruye en cada request**, y no puede tocar tu código. Si lo único que exponés para obtener una instancia es la vía de creación —la que inventa una identidad nueva—, reconstruir una entidad guardada le cambia el identificador: el id que el cliente recibió deja de existir, y completar, eliminar o relacionar esa entidad contesta *"no existe"*. Exponé una vía de reconstitución **separada de la de creación**, que reciba la identidad ya existente junto con el estado completo — incluidos el estado del ciclo de vida y las colecciones que la entidad haya acumulado. Lo que tengas `internal` para proteger la invariante durante la vida normal de la entidad necesita su equivalente accesible acá, o la API no puede restaurar lo que ella misma guardó. **Es un fallo de runtime que compila y pasa todos los gates**, y ya rompió una corrida real: la reconstitución es la frontera donde este pipeline falla, no las reglas de negocio.
